...
def recommend_final(BreedIWant, my_condition):
    ...
    if len(final) < 15:  # 若沒有補滿15個推薦，則都推薦混種
        ...
        if my_condition["Type"] == '狗':
            test5 = (df4['pet_category'] == '狗')
            df4 = df4[test5]
            # test6 = (df5['pet_variety'] == '米克斯')
            # df6 = df5[test6].sample(n=15 - len(final))
        elif my_condition["Type"] == '貓':
            test5 = (df4['pet_category'] == '貓')
            df4 = df4[test5]
            # test6 = (df5['pet_variety'] == '米克斯')
            # df6 = df5[test6].sample(n=15 - len(final))
        # else:
        #     df5 = (df4[[('混種' in x) for x in df4['pet_variety']]]).sample(n=15 - len(final))
        test6 = (df4['pet_variety'] == '米克斯')
        df6 = df4[test6].sample(n=15 - len(final))
        final += df6.to_dict('records')
...
def recommend_insert_sql(data):
    ...
    try:
        data_insert_sql = f"INSERT INTO `recommend` (`user_id`,`recommend_pets`) VALUES ('{values[0]}','{values[1]}');"
        engine.execute(text(data_insert_sql))
    except:
        data_update_sql = f"UPDATE `recommend` SET `recommend_pets` = '{values[1]}' WHERE `user_id`='{values[0]}';"
        engine.execute(text(data_update_sql))



def recommend_main(id):
    sql_commend = f'select * from `userinfo` where user_id="{id}"'
    mysql_engine = connect_my_sql()
    df0 = pd.read_sql(sql=sql_commend, con=mysql_engine)
